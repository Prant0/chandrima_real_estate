...
class UpdateTenantScreen extends StatefulWidget {
  ...
  @override
  State<UpdateTenantScreen> createState() => _UpdateTenantScreenState();
}

class _UpdateTenantScreenState extends State<UpdateTenantScreen> {
  ...
  void initState() {
    super.initState();
    // Pre-fill the fields with the tenant data
    _nameController.text = widget.tenants.name ?? '';
    _phoneController.text = widget.tenants.mobile ?? '';
    _emailController.text = widget.tenants.email ?? '';
    _houseNoController.text = widget.tenants.houseNumber ?? '';
    ...
    Get.find<ProfileController>().setSelectedGender(widget.tenants.gender!, isUpdate: false);
  }

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: const CustomAppBar(title: 'Update Tenant'),
          ...
          children: [
                          ...
                          Expanded(
                            child: Padding(
                              ...
                              child: Text(
                                rentYear!.isEmpty
                                    ? 'Select Rent Date'
                                    : "Month : $rentMonth, Year : ${rentYear!}",
                                style: poppinsRegular.copyWith(color: AppColors.black, fontSize: 15),
                              ),
                            ),
                          ),
                          IconButton(
            ...
            CustomCard(
              ...
              child: CustomButton(
                ...
                buttonText: 'Update Tenant',
                onPressed: () {
                  profileController.updateTenant(
                    id: widget.tenants.id!,
